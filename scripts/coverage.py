...
def coverage_area(latitude_interval):
    coverage_data = []
    for angles in range(0, 91, latitude_interval):  # np.sin(np.radians(60))
        latitude2 = angles
        ...
        eccentricity = 1-((earth_radius_poles**2)/(radius_at_latitude**2))
        area = 4*np.pi*radius_at_latitude**2
        if eccentricity != 0:
            earth_surface_area_at_latitude = (2*radius_at_latitude**2)*(
                1+(((1-(eccentricity**2))/eccentricity)*(mt.atanh(eccentricity))))
